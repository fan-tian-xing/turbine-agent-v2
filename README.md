...
阶段 11 已冻结 Engineering Statement 合同和唯一 Evaluation Sample Registry，但页面与 Evidence 当前仍是待独立语义复核的候选。36 页开发/回归 Golden（其中包含阶段 3 的 15 页试点子集）与五份资料各 3 页的 15 页 Statement 留出集分开登记；留出集只由评价入口读取，盲测内容不由本项目读取。两轮独立审核建议已分别保存，拆分或结论冲突进入 adjudication queue；在裁决完成前 Stage 12 保持关闭。构建和退出检查见 [`data/stage11/README.md`](data/stage11/README.md)。
