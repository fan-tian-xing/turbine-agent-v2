...
阶段 7 的术语输入清单覆盖首批五个供给单元的 775 个物理页，但只消费其中明确通过页级门禁的 `text_accepted` 页面；`stage7/terminology_candidates.json` 和业务能力问题均为 `candidate_only`，不得当作本体、正式词汇或 Release。`stage7/terminology_review_queue.jsonl`、`stage7/terminology_relationship_review_queue.jsonl` 和 `stage7/stage7_human_review_summary.json` 是人工审核入口，审核完成前阶段 8 被阻塞。
