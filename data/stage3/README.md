...
本目录记录阶段 3 的研发验证产物，不是正式 Release，不保存私有盲测题或原始资料全文；保留研发复核所需的最小 Evidence 摘录与追溯元数据。项目当前状态唯一以 `../project_state.json` 为准，阶段 3 的门禁证据见 `stage3_exit_audit.json`。

主要产物：

- `initial_batch_manifest.json`：从阶段 2B 已准入来源中冻结代表页的试点采样入口；
- `review_metrics.json`：人工审核结果和错误类别记录；
- `real_trial_summary.json`：真实试点页面的抽取摘要；
- `real_trial_confirmation.json`：用户确认的真实页面 Evidence/Statement 及其 Asset/Revision/Page/SourceSpan 链接；
- `real_trial_benchmark.json`：已确认真实案例的 JSON baseline/图投影候选集对照；
- `real_trial_execution.json`：代码重放真实确认案例后的检索、Claim 校验和延迟结果；
...
- `baseline_comparison.json`：Fixture 问题及真实确认案例的 baseline/投影一致性记录；
...
研发闭环覆盖页面抽取、真实 Evidence 确认、隔离 Neo4j 试点投影，以及中文问题的来源、页码和 Evidence 回查；执行数量以对应 Manifest、确认和回放记录为准。LLM 适配器只接受带 Evidence ID、页码、对象、适用条件和 Claim 类型的结构化回答；真实重放、Claim 校验和最终答案组装均保留在本目录的审计与回放记录中。外部 LLM 只接收检索到的最小证据片段。正式首批处理仍需遵守阶段 15 的准入门禁。
