...
def _human_result(result: dict) -> None:
    ...
    for hit in result["hits"]:
        ...
        page = f"第{hit['page']}页" if hit.get("page") else "页码未确认"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("command", choices=("import", "status", "ask"))
...
def main() -> int:
    ...
    with TrialGraph(settings) as graph:
        ...
        if args.command == "status":
            ...
            return 0
        question = args.question or args.question_positional
        if not question:
            ...
            "hits": [
                {
                    ...
                    "page": hit["sources"][0]["page"].get("page_number") if hit["sources"] else None,
