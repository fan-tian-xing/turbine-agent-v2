...
- `stage4_smoke_2026-09-08.json`：15 页、5 个已登记资产的真实链路冒烟记录；`stage4_full_parse_audit_2026-09-09.json`：五份冻结处理单元的 775 页全量结构解析、哈希、页序映射和异常页清单；`stage4_final_exit_audit_2026-09-09.json`：最终退出审计；`stage4_exit_audit.json`：阶段 4 退出汇总，状态为 `complete/closed`；`stage4_dead_code_orphan_audit.json`：阶段 4 范围内的死代码和孤立产物审查。
- 当前文档层回归测试共 25 项，项目全量测试数量以最近一次完整 pytest 运行记录为准。
