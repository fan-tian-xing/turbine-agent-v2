...
OCR_DERIVED_ROOT = (PROJECT_ROOT / "var" / "derived" / "ocr").resolve()
sys.path.insert(0, str(PROJECT_ROOT / "scripts"))

from turbine_kg.registry.identity import asset_id_for_path, load_document_identity_map
from turbine_kg.registry.source_inputs import load_allowlist
...
def test_registry_counts_and_identity_contract():
    ...
    assert all(asset["revision_id"].startswith("rev-") for asset in assets)
    assert all(asset["asset_kind"] in {"original", "derived_ocr"} for asset in assets)
    assert all(asset["source_root_id"] in {"source", "ocr_derived"} for asset in assets)
    for document in documents:
        assert document["revision_ids"]
    for asset in assets:
...
def test_stage2_audit_outputs_exist():
    ...
    assert selection["stage"] == "2B"
    assert selection["stage_status"] == "complete"
    assert selection["completion_scope"] == "stage3_trial_source_selection"
